...
def visualize(time, measured_Q, predicted_Q, corrected_Q):
    """
    ...
    plt.grid(True)
    plt.savefig("Q_results.png", dpi=300)
    plt.show()

def train_gp_correction_for_Q(thermal_model, time_horizon, Tamb_t_list, Tin_t_list, Qin_t_list, step_pre, vent_flow, Q_space_measured):
    """
    ...
    # 可视化结果
    visualize(time_hour, Q_space_measured[:len(Qspace_list)], Qspace_list, corrected_Q)
