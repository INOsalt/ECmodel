...
# 完整灰箱模型
def grey_box_model(params, time, external_temp, Q_heat, vent_temp, vent_flow):
    """
    ...
    wall_temp[0] = external_temp[0]  # 初始墙体温度
    internal_temp[0] = 22  # 初始室内空气温度
    ...
    for t in range(1, len(time)):
        ...
        try:
            ...
            print(f"Overflow encountered at t={t}, setting Q_vent to 0.")
            Q_vent = 0


        # 6. 室内空气温度动态更新
        d_internal_temp = (
            Q_zone_wall + Q_zone_rad + Q_vent
        ) / C_zone  # 将所有热流项除以热容
...
# 参数辨识目标函数
def objective_function(params, time, external_temp, Q_heat, vent_temp, vent_flow, measured_temp):
    """
    目标函数：最小化灰箱模型预测值与测量值之间的均方误差。
    """
    # 使用当前参数调用灰箱模型
    predicted_temp = grey_box_model(params, time, external_temp, Q_heat, vent_temp, vent_flow)
...
# 参数辨识方法
def parameter_identification(time, external_temp, Q_heat, internal_temp, vent_temp, vent_flow):
    """
    参数辨识：通过最小化误差优化模型参数。
    输入：
    - time: 时间序列 (小时)
    ...
    initial_params = [10, 10, 1000, 500]

    # 参数取值范围
    bounds = [
        (0.5, 100000),  # R_ext_wall
        (0.5, 100000),  # R_zone_wall
        (10, 100000),  # C_wall
        (10, 100000)  # C_zone
    ]
    ...
    result = minimize(
        ...
        initial_params,  # 初始参数
        args=(time, external_temp, Q_heat, vent_temp, vent_flow, internal_temp),  # 其他参数
        bounds=bounds,  # 参数范围
...
def main():
    ...
    time, external_temp, radiator_power, vent_temp, vent_flow, internal_temp = load_real_data(file_path)
